...
|------|------|------|------|-----------|------|
| 🧭 **工作台** | 个人工作台 | `/dashboard` | ✅ 已实现 | Phase 3 | 统计卡片 + 最近活动 + 我的操作 + 快捷入口，登录默认首页 |
| 🧭 **工作台** | 全局搜索 | 顶部搜索框 | ⏳ 待开发（**Phase 3**） | Phase 3 | 跨用户/角色/部门/审计/知识一键搜索 |
| 🧭 **工作台** | 通知中心 | 铃铛 + `/notifications` | ⏳ 待开发（**Phase 3**） | Phase 3 | 操作通知 + 未读 badge + 已读管理 |
| 🧭 **工作台** | 密码过期/会话超时 | — | ✅ 已实现 | Phase 3 | 密码过期强制改密 + idle 超时自动登出 + JWT pwd_exp claim |
| 📊 **分析** | 报表中心 | `/report` | ✅ 已实现 | Phase 3 | 报表矩阵总览 + 登录安全/敏感操作/合规健康 3 张报表 + ECharts 可视化 + Excel 导出 |
...
| 🔗 **融合** | 资产管理 | `/assets` | ✅ 已实现 | Phase 4 | CMDB 配置管理：5 种 CI 模型 + 动态属性 + 关系拓扑 + CRUD |
...
> 对比对象：腾讯蓝鲸智云 / 华为云 COC / 嘉为蓝鲸 WeOps / ServiceNow ITOM / MeridianOps
> 更新时间：2026-08-12
> 图例：✅ 完整支持 / ⚠️ 部分支持 / ⏳ 规划中 / ❌ 不支持 / — 非核心
...
| 功能项 | 腾讯蓝鲸 | 华为云 COC | 嘉为 WeOps | ServiceNow | **MeridianOps** |
|--------|---------|-----------|-----------|-----------|-----------------|
| 统一身份认证（本地/SSO） | ✅ | ✅ | ✅ | ✅ | ✅ Argon2 + 2oauth SSO 预留 |
| 多租户隔离 | ✅ | ✅ | ✅ | ✅ | ⏳ Phase 7 |
| RBAC 细粒度权限 | ✅ | ✅ | ✅ | ✅ | ✅ 18 权限点 + 3 级角色 |
| 部门树形组织 | ✅ | ✅ | ✅ | ✅ | ✅ parent_id 自引用 + 删除保护 |
| 操作审计（自动留痕） | ✅ | ✅ | ✅ | ✅ | ✅ 中间件自动记录 + 分页筛选 |
| 密码强度策略 | ✅ | ✅ | ✅ | ✅ | ✅ 可配置 5 维度策略 |
| 登录失败锁定 | ✅ | ✅ | ✅ | ✅ | ✅ 5 次锁 15 分钟（可调） |
| 密码过期 / 强制改密 | ✅ | ✅ | ✅ | ✅ | ✅ JWT pwd_exp claim |
| 会话超时（idle） | ✅ | ✅ | ✅ | ✅ | ✅ 60 分钟默认（可调） |
| TOTP 双因子登录 | ✅ | ✅ | ✅ | ✅ | ⏳ Phase 4 |
| 双人审批（四眼原则） | ✅ | ✅ | ✅ | ✅ | ⏳ Phase 4 审批中心 |
| 堡垒机操作记录回流 | ⚠️ 需对接 | ✅ | ⚠️ 需对接 | ⚠️ 需对接 | ⏳ Phase 5（帕拉迪） |
| 等保三级合规报告 | ✅ | ✅ | ✅ | ✅ | ⏳ Phase 5 报表中心 |

#### 2. 数据融合与 CMDB

| 功能项 | 腾讯蓝鲸 | 华为云 COC | 嘉为 WeOps | ServiceNow | **MeridianOps** |
|--------|---------|-----------|-----------|-----------|-----------------|
| 自研 CMDB 配置管理 | ✅ CI 模型 + 属性 + 关系 | ✅ | ✅ | ✅ Service Mapping | ✅ 5 种 CI 模型 + 动态属性 + 关系 |
| CI 自动发现 | ✅ 节点管理 Agent | ✅ | ✅ | ✅ Discovery | ⏳ Phase 6 Agent 管理 |
| **外部 CMDB 同步接口（Webhook）** | — | ⚠️ | ⚠️ | ✅ Import Sets | ✅ **POST /api/cmdb/sync（蓝鲸已对接）** |
| 外部 CMDB 拉取模式（API） | ✅ 蓝鲸集成 | ✅ | ✅ | ✅ LDAP/REST | ⏳ Phase 5 同步数据源配置 |
| 多源告警接入（自研） | ✅ BKM 接入层 | ✅ | ✅ | ✅ Event Management | ❌ **不自研，统一对接 Eventide** |
| 告警统一去重降噪 | ✅ | ✅ | ✅ | ✅ | ⏳ Phase 6（Eventide 侧已做） |
| 日志检索 / 分析 | ✅ BKL | ✅ | ✅ | ✅ Log Observer | ⚠️ 直连 ELK（iframe + 跳转） |
| 跨源关联（告警 × 资产 × 发布） | ⚠️ | ✅ | ⚠️ | ✅ | ⏳ Phase 6 拓扑视图 |
| 统一拓扑视图 | ✅ bk-topo | ✅ | ✅ | ✅ Service Map | ⏳ Phase 6 业务→主机两层 |

#### 3. 自动化与流程

| 功能项 | 腾讯蓝鲸 | 华为云 COC | 嘉为 WeOps | ServiceNow | **MeridianOps** |
|--------|---------|-----------|-----------|-----------|-----------------|
| 作业平台（脚本/文件/定时） | ✅ JOBs | ✅ | ✅ | ✅ Orchestration | ⏳ Phase 4 作业中心 |
| 流程编排（可视化） | ✅ bk-itsm 流程 | ✅ | ✅ | ✅ Flow Designer | ⏳ Phase 7 |
| ITSM 工单全生命周期 | ✅ 事件/问题/变更/发布 | ✅ | ✅ 事件/问题/变更/发布/配置 | ✅ Incident/Problem/Change/RITM | ⏳ Phase 4 工单系统 |
| SLA 管理 / 自动升级 | ✅ | ✅ | ✅ | ✅ | ⏳ Phase 5 |
| 高危命令拦截 / 变更风控 | ✅ | ✅ WarRoom | ✅ | ✅ | ⏳ Phase 4（审批中心） |
| 标准作业剧本库 | ✅ 公共剧本 | ✅ | ✅ | ✅ | ⏳ Phase 4 预置 5 个高频剧本 |
| 自动自愈（告警→执行） | ✅ | ✅ | ✅ | ✅ Automation | ⏳ Phase 7 AIOps 自愈编排 |
| 服务目录 / 自助门户 | ✅ | ✅ | ✅ | ✅ Service Catalog | ⏳ Phase 5 |
| 远程 Web 终端 | ✅ | ✅ | ✅ | ⚠️ | ⏳ Phase 6 |
| 容器管理（K8s） | ✅ bk-bcs | ✅ CCE | ✅ | ✅ | ⏳ Phase 5 接 K8s API |

#### 4. 智能化与分析

| 功能项 | 腾讯蓝鲸 | 华为云 COC | 嘉为 WeOps | ServiceNow | **MeridianOps** |
|--------|---------|-----------|-----------|-----------|-----------------|
| 个人工作台 / 值班首页 | ✅ | ✅ | ✅ | ✅ | ✅ 统计 + 活动 + 快捷入口 |
| 全局搜索 | ✅ | ✅ | ✅ | ✅ | ⏳ Phase 3 |
| 态势大屏 / 数据看板 | ✅ 数据可视化平台 | ✅ | ✅ | ✅ Performance Analytics | ⏳ Phase 5 |
| MTTR / 可用率报表 | ✅ | ✅ | ✅ | ✅ | ✅ 报表中心 3 张核心 + 导出 |
| 通知中心（多通道） | ✅ 邮件/短信/IM | ✅ | ✅ 邮件/短信/飞书/钉钉 | ✅ 多通道 | ⏳ Phase 3 站内信 + Phase 5 外部 |
| 根因分析 RCA | ⚠️ 需 AIOps 套件 | ✅ | ⚠️ | ✅ AIOps | ⏳ Phase 7 |
| 相似告警推荐知识 | ❌ 需购买 AI 套件 | ⚠️ | ❌ | ✅ AI Search | ⏳ Phase 7 |
| 异常趋势检测 / 预测 | ⚠️ 需 AIOps 套件 | ✅ | ❌ | ✅ Predictive AIOps | ⏳ Phase 7 |
| 知识库（版本 + 检索） | ✅ bk-wiki | ✅ | ✅ | ✅ Knowledge | ⏳ Phase 4 |
| 成本分析 / FinOps | ⚠️ | ✅ 成本中心 | ⚠️ | ✅ | ⏳ Phase 5 费用中心 |

---

### 二、技术栈对比矩阵

| 维度 | 腾讯蓝鲸 | 华为云 COC | 嘉为 WeOps | ServiceNow | **MeridianOps** |
|------|---------|-----------|-----------|-----------|-----------------|
| 后端语言 | Python + Go 多套 | Java | Java | 闭源（Java/自研） | **Rust（单语言）** |
| 前端框架 | Vue 2 + MagicBox | Vue 3 | Vue 3 | 闭源（Aura UI / Now UI） | **Vue 3 + TypeScript** |
| 数据库 | MySQL + PostgreSQL + Redis + MongoDB + ES | MySQL（华为内部兼容） | MySQL | MySQL | **仅 MySQL（单库）** |
| 消息队列 | RabbitMQ + Kafka | Kafka | RabbitMQ | 闭源 | 无（Phase 6 按需引入） |
| 部署复杂度 | ★★★★★（10+ 组件） | ★★★★☆（托管服务） | ★★★★☆（5+ 组件） | ★☆☆☆☆（SaaS 开箱） | **★☆☆☆☆（二进制 + npm build）** |
| 最小部署资源 | 8 核 32G + 100G | 按需（公有云） | 4 核 16G + 80G | SaaS 无要求 | **2 核 4G + 20G 即可跑** |
| 运行时依赖 | Python 3.6/3.8 + JDK + Node.js | JDK 11 + Node.js | JDK 8 + Node.js | 无（浏览器） | **Rust 静态链接，无运行时** |
| 插件扩展 | 插件市场（SaaS） | 华为 AppGallery | 插件包 | Store 应用市场 | **无（预留 Adapter 骨架）** |
| 开源协议 | MIT（主版） | 闭源（公有云） | 商业许可 | 商业 SaaS | **MIT（全栈开源）** |
| 二次开发成本 | ★★★★☆（需 Python/Go 专家） | ★★★☆☆（有 API） | ★★★☆☆（有 SDK） | ★★★★★（需 ServiceNow 认证专家） | **★☆☆☆☆（Rust + Vue 全栈工程师可上手）** |

---

### 三、银行场景适配对比

> 面向银行/证券/保险等强合规行业的落地能力评估

| 维度 | 腾讯蓝鲸 | 华为云 COC | 嘉为 WeOps | ServiceNow | **MeridianOps** |
|------|---------|-----------|-----------|-----------|-----------------|
| **数据不出行（私有化部署）** | ✅ 支持 | ⚠️ 仅限华为云专有云 | ✅ 支持 | ❌ 主要 SaaS（私有版极贵） | **✅ 纯私有化，数据全在本地** |
| 银行业务拓扑（业务→主机→中间件→数据库） | ⚠️ 需定制 | ⚠️ 通用模型 | ✅ 内置金融模型 | ⚠️ 需付费 Service Mapping | **✅ 内置 5 种 CI 模型，映射即用** |
| 对接堡垒机（帕拉迪/齐治） | ⚠️ 需开发 Adapter | ✅ 华为自带 | ✅ 帕拉迪深度合作 | ❌ 海外体系 | ⏳ Phase 5 对接预留 |
| 对接短信网关（移动/联通） | ⚠️ 需配置 | ✅ 华为云短信 | ✅ | ❌ 海外短信 | ⏳ Phase 5 中国移动对接预留 |
| 等保三级审计留痕 | ✅ 完整 | ✅ | ✅ | ✅ | ✅ 完整（中间件自动 + 人工不可删） |
| 国产化适配（鲲鹏/麒麟/达梦） | ✅ | ✅ 华为生态 | ✅ | ❌ 不支持 | ⏳ Phase 7 |
| 行内 SSO（2oauth/OA 统一认证） | ⚠️ 定制开发 | ✅ 华为统一认证 | ✅ 多 SSO | ❌ 需 IDP 对接 | ⏳ Phase 6 2oauth 对接预留 |
| ITIL 流程（优云/北塔/国迈）对接 | ⚠️ 需定制 | ✅ 华为体系 | ✅ 优云深度 | ❌ 需付费集成 | ⏳ Phase 4 优云 ITIL 对接预留 |
| **CMDB 对接（蓝鲸/优云）** | — 内部互通 | ⚠️ 需定制 | ✅ 优云互通 | ❌ 需专业服务 | **✅ 蓝鲸 webhook 已落地 + 优云 API 预留** |

---

### 四、差异化定位（核心竞争力总结）

与上述 4 个主流平台相比，MeridianOps 的差异化在于 **"融合层 + 轻量 + 银行定制"** 三重定位：

| 差异化维度 | 说明 | 竞品状况 |
|-----------|------|---------|
| **1. 融合而非再造** | 不重复造监控/日志/作业轮子，直接对接行内已有的 Eventide（告警）/ AxleOps（作业）/ ELK（日志），做跨源关联与统一体验 | 蓝鲸/嘉为等多主张"全栈替换"，导致银行已有系统重复建设、迁移成本巨大 |
| **2. 银行场景开箱即用** | CI 模型预置"业务系统 → 主机 → 中间件 → 数据库 → 网络设备"银行标准拓扑；蓝鲸 CMDB Webhook 接口已落地 | 通用平台需要大量定制建模、写 Adapter |
| **3. 技术栈极简统一** | 后端 Rust（单二进制无依赖）+ 前端 Vue 3 + 单 MySQL 数据库，部署资源 < 蓝鲸 1/10；全栈工程师即可维护 | 蓝鲸至少 5 种语言 + 10+ 组件；嘉为/华为 Java 体系 + Kafka/Rabbit/Redis/ES |
| **4. 数据主权自主** | CMDB/权限/审计/RBAC 一体化在自有 MySQL 中，审计日志中间件强制留痕不可绕过 | SaaS 平台（ServiceNow/华为云）数据在第三方；开源平台组件间权限割裂 |
| **5. 外部 CMDB 双向同步** | 已实现蓝鲸 webhook 推送模式：按 `(source, external_id)` 幂等 upsert，同步日志完整快照 + 批次审计；预留拉取模式接口 | 通用平台需付费购买"集成包"或开发 Agent |

### 五、参考平台资料

| 平台 | 参考价值 | 官方链接 |
|------|---------|---------|
| 腾讯蓝鲸智云 | 套餐化架构 / CMDB / 作业平台 / 流程编排开源标杆 | https://bk.tencent.com/ |
| 华为云 COC | 一站式云运维 / 变更风控 / WarRoom 协同 | https://www.huaweicloud.com/zh-cn/product/coc.html |
| 嘉为蓝鲸 WeOps | 监管控营服一体化 / 金融行业落地 / 故障全生命周期 | https://www.canway.net/WeOps/791.html |
| ServiceNow ITOM | 全球 ITSM / CMDB / AIOps 事实标准 | https://www.servicenow.com/products/it-operations-management.html |
...
- [ ] 未完成/空壳模块（态势真数前的空壳、容器、库、配置、费用等）：菜单隐藏或标「规划中」，避免误触 — **产品** `P0`
- [ ] 凭据/JWT/种子密码勿硬编码；生产默认 JWT 密钥应阻断启动（不仅 warn）— **架构/测试** `P0`
- [ ] 前端 Token 不要走 URL — **架构** `P0`
...
- [ ] CORS 全开收紧 — **架构** `P1`
- [ ] 告警静默规则竞态修复 — **架构/测构** `P1`
- [ ] SSH host key 跳过校验：演示环境标风险，不上生产默认 — **架构/测试** `P1`

### 🟢 本轮延后（明确标延后，勿删）

- [ ] `db.rs` God 文件拆分 — **架构** `P2`
- [ ] 调度器缺分布式锁 — **架构** `P2`
- [ ] ClickHouse 无超时 — **架构** `P2`
- [ ] 路由守卫过重 — **架构** `P2`
- [ ] 产品：态势接真数据、全局搜索 MVP（下一轮产品 P0）— **产品** `P2`
- [ ] 产品：通知外发、空状态统一、工作流可视化、移动端 — **产品** `P2`
- [ ] 测试：补充自动化测试框架与冒烟用例（硬门槛过后再写冒烟：登录锁定/改密/RBAC 403/审计留痕/告警静默 + 菜单隐藏回归）— **测试** `P2`
