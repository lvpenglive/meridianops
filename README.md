...
- [ ] SSH 跳过主机密钥校验：演示环境标风险，不上生产默认 — **架构** `P1`
- [ ] localStorage 存 Token（XSS 可窃取）：演示环境标风险，生产需改 httpOnly cookie — **架构** `P1`

### 🟠 延后 P1（高优先级，下轮处理）

- [ ] `db.rs` God 文件拆分（3211 行）— **架构**
- [ ] 工单调度器缺分布式锁 — **架构**
- [ ] ClickHouse 查询无超时 — **架构**
- [ ] SSH 执行器/通知发送缺重试队列 — **架构**
- [ ] 路由守卫职责过重 — **架构**
...
- [ ] API 限流/熔断/重试机制 — **架构**
- [ ] 健康检查加依赖探测（DB/Redis/外部服务）— **架构**
- [ ] 连接池配置优化 & 指标收集 — **架构**
- [ ] 测试覆盖补充（当前接近零）— **测试**
- [ ] 产品：态势接真数据、全局搜索 MVP（下一轮产品 P0）— **产品**
- [ ] 产品：通知外发、空状态统一、工作流可视化、移动端 — **产品**
- [ ] 测试：补充自动化测试框架与冒烟用例（硬门槛过后再写）— **测试**
