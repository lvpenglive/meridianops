-- 20260816000024: 修复作业中心权限种子（兼容迁移23部分执行的情况）
-- 先确保三个作业核心表存在（若迁移23的建表部分未执行则兜底创建）
CREATE TABLE IF NOT EXISTS job_definitions (
    ...
    target_asset_ids JSON        NULL COMMENT '静态资产ID列表',
    target_cmdb_query VARCHAR(1024) NOT NULL DEFAULT '' COMMENT 'CMDB 查询条件',
    run_as          VARCHAR(128) NOT NULL DEFAULT 'root' COMMENT '执行用户',
